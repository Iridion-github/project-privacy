import fs from 'fs'//pacchetto usato per docx files
import path from 'path'
import slash from 'slash'
import { PdfReader } from "pdfreader"  //pacchetto usato per pdf files
import mammoth from 'mammoth' //pacchetto usato per pdf files
import Blob from "cross-blob"
import StreamZip from 'node-stream-zip' //pacchetto usato per doc files
        ...
        const singleFilePromise = new Promise((resolveSingle, rejectSingle) => {
          try {
            if (pdf) { //[Pdf procedure]
              ...
              if (docx) { //[Docxx procedure]
                const docxContent = fs.readFileSync(fileObj.fullpath, 'utf8')
                const docxBuffer = fs.readFileSync(fileObj.fullpath)
                ...
                const docProcedure = new Promise((resolveDoc, rejectDoc) => {
                  try {
                    const zip = new StreamZip.async({ file: fileObj.fullpath })
                    setTimeout(() => resolveDoc(zip), 1000)
                  } catch (errDoc) {
                    rejectDoc(errDoc)
                  }
                }).then((zipResult) => {
                  console.log("\n\n zipResult:", zipResult, "\n\n")
                  const entriesCount = zipResult.entriesCount
                  return entriesCount
