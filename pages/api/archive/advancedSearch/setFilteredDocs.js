...
export const setFilteredDocs = async ({
  envSlash,
  refreshRate,
  globalState,
  updateGlobalState,
  ...
  const getFilteredDocs = async () => {
    const filteredArr = await globalState.dataToFilter.filter(async (d, filterIndex) => {
      if (d.content) {
        ...
        if (globalState.mustBeProvv && !d.fullpath.includes(envSlash + "Provvedimenti" + envSlash)) {
          ...
          if (d.filename.includes(".pdf")) {
            //condizione momentanea per testare se almeno coi pdf funziona
            console.log(">>>>>>>>>>>>>>>>>>>>>> Start updating updateByProvvedimentoState.resultsByArticle");
            await findArticolo({
              enterPath: d.fullpath,
              target: byProvvedimentoState.target,
              requiredFormat: byProvvedimentoState.requiredFormat,
              updateByProvvedimentoState: updateByProvvedimentoState,
            });
            while (byProvvedimentoState.resultsByArticle === undefined) {
              await delay(500);
              console.log("byProvvedimentoState.resultsByArticle is still undefined, looping ...");
            }
            console.log("byProvvedimentoState.resultsByArticle:", byProvvedimentoState.resultsByArticle);
            console.log(">>>>>>>>>>>>>>>>>>>>>> DONE updating updateByProvvedimentoState.resultsByArticle");
          }
    ...
    console.log("ooooooooooooo setting getFilteredDocsState.filteredArr to this:", filteredArr);
    await updateGetFilteredDocsState("filteredArr", filteredArr);
