...
module.exports = __webpack_require__("Ua2N");
...
/* harmony default export */ __webpack_exports__["default"] = (async (req, res) => {
  ...
  let targetTest = await _models_Test__WEBPACK_IMPORTED_MODULE_1__[/* default */ "a"].findOne({
    id: id
  });
  if (!targetTest) return res.status(400).json({
    success: false,
    error: "No test found for that id!"
  });
  ...
  switch (method) {
    ...
    case "PUT":
      try {
        ...
        return res.status(400).json({
          success: false,
          error: "Failed update of existing test!"
        });
      }

    case "DELETE":
      try {
        await _models_Test__WEBPACK_IMPORTED_MODULE_1__[/* default */ "a"].deleteOne({
          _id: targetTest.id
        });
        await targetTest.save();
        return res.status(200).json({
