...
export const ArticlesList = function (props) {
  ...
  const articlesRows = useMemo(
    ...
    [elementsPerRow, props.filtered, props.allArticles]
  );
  ...
  return (
    <Row className="">
      ...
      {visibleRows.length > 0 && (
        <MyPagination
          totalPages={
            !props.filtered ? Math.ceil(props.allArticles.length / elementsPerPage) : Math.ceil(props.searchFilter(props.allArticles, props.searchInput, props.currentLang).length / elementsPerPage)
          }
      ...
      <Row className="w-100 mobile-compatible m-auto">
        <Col md={12} className={visibleRows.length > 0 ? "resulting-article-container" : "d-flex resulting-article-container align-items-center justify-content-center"}>
          {visibleRows.length > 0 ? (
            visibleRows.map((row, i) => <ArticlesRow setOpenedArticle={props.setOpenedArticle} key={i} articles={row} />)
          ) : (
