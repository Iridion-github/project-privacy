export const ResultRow = function (props) {
  const userAnswerSpan = Array.isArray(props.userAnswer) && props.userAnswer.length > 1 ? props.userAnswer.map((elem, i) => <span key={i}> {elem.text} | </span>) : props.userAnswer;
  const correctAnswerSpan =
    props.correctAnswer.length > 1
      ? props.correctAnswer.map((elem, i) => (
          <span key={i}>
            {" "}
            {i !== 0 ? "|" : ""} {elem.text}{" "}
          </span>
        ))
      : props.correctAnswer[0].text;
  return (
