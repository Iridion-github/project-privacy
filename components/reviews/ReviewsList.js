...
export const ReviewsList = function (props) {
  ...
  const visibleRows = reviewsRows.filter(
    (elem, index) => index >= (currentPage * elementsPerPage) / elementsPerRow - elementsPerPage / elementsPerRow && index < (currentPage * elementsPerPage) / elementsPerRow
  );
  ...
  return (
    <Row className="">
      ...
      {visibleRows.length > 0 && (
        <MyPagination
          totalPages={
            !props.filtered ? Math.ceil(props.allReviews.length / elementsPerPage) : Math.ceil(props.searchFilter(props.allReviews, props.searchInput, props.currentLang).length / elementsPerPage)
          }
      ...
      <Row className="w-100 mobile-compatible m-auto">
        <Col md={12} className={visibleRows.length > 0 ? "resulting-article-container" : "d-flex resulting-article-container align-items-center justify-content-center"}>
          {visibleRows.length > 0 ? (
            visibleRows.map((row, i) => <ReviewsRow setOpenedReview={props.setOpenedReview} key={i} reviews={row} />)
          ) : (
